...
class FullyConnected:
  def __init__(self, sizes=[1, 1], alpha=0.1):
    ...
    for i in range(len(self.layersizes)):
      self.w.append(tf.Variable(tf.truncated_normal(
        [interim_size, self.layersizes[i]], stddev=0.1),
        name="l" + str(i) + "w"))
      self.b.append(tf.Variable(tf.constant(0.1, shape=[self.layersizes[i]]),
        name="l" + str(i) + "b"))
      interim_size = self.layersizes[i]
  ...
  def train(self, dataset, num_epochs=1):
    ...
    for i in range(num_batches):
      ...
      rewards = dataset["values"][idx[batch_id:new_batch_id], :]
