...
class Slider():
  def __init__(self, value_range, pos, shape, buttonColor=(0x03, 0xA9, 0xF4), \
      fillColor=(0xF5, 0xF5, 0xF5), lineColor=(0xBD, 0xBD, 0xBD), radius=None, initialValue=0):
    ...
    self.length = max(self.shape[0], self.shape[1]) - min(self.shape[0], self.shape[1])
    self.slide_render = pygame.Surface(self.shape)
    self.slide_render.fill(fillColor)
    if self.shape[1] > self.shape[0]:
      pygame.draw.line(self.slide_render, lineColor, (self.pad / 2, self.pad / 2),
  ...
  def render(self, surface):
    ...
    ratio = (self.value - self.value_range[0]) / (self.value_range[1] - self.value_range[0])
    if self.reversed:
      ratio = 1.0 - ratio
    ...
    if self.shape[1] > self.shape[0]:
      pygame.draw.line(highlight, self.buttonColor, (self.pad // 2, self.pad // 2),
  ...
  def update(self, mousepos):
    ...
    if self.reversed:
      ratio = 1.0 - ratio
    self.value = ratio * (self.value_range[1] - self.value_range[0]) + self.value_range[0]
...
class TextBox():
  def __init__(self, pos, shape, fontColor=(0x03, 0xA9, 0xF4), fillColor=(0xF5, 0xF5, 0xF5), \
      initialValue="0", fmt="%s"):
    self.pos = pos
    self.shape = shape
...
class SliderCounter():
  def __init__(self, value_range, pos, shape, radius=None, counterWidth=None, fmt="%s", initialValue=0):
    if counterWidth == None:
      counterWidth = shape[1]
    self.slider = Slider(value_range, pos, (shape[0] - counterWidth, shape[1]), radius=radius, initialValue=initialValue)
    self.counter = TextBox((shape[0] - counterWidth + pos[0], pos[1]), (counterWidth, shape[1]), fmt=fmt, initialValue=initialValue)
